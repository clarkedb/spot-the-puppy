...
describe('/api/high-score', () => {
  ...
  describe('GET', () => {
    ...
    it('should handle datastore errors gracefully', async () => {
      vi.mocked(datastore.getHighScore).mockRejectedValue(new Error('Database error'))
      ...
      expect(NextResponse.json).toHaveBeenCalledWith({ error: 'Invalid request' }, { status: 400 })
    })
    ...
    it('should handle URL encoding in player names', async () => {
      ...
      expect(datastore.updateHighScore).toHaveBeenCalledWith(1500, 'Player One')
      expect(NextResponse.json).toHaveBeenCalledWith(newScore)
    })
