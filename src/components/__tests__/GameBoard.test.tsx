...
vi.mock('../GameSprite', () => ({
  default: ({ type, onPuppyClick }: { type: string; onPuppyClick?: () => void }) => (
    <div data-testid={`game-sprite-${type}`} onClick={type === 'puppy' ? onPuppyClick : undefined}>
      {type} sprite
    </div>
  ),
}))
...
describe('GameBoard', () => {
  ...
  it('should not render when not visible', () => {
    const { container } = render(<GameBoard {...defaultProps} isVisible={false} />)
