...
export default function GameSprite({
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      if (containerRef.current) {
        setPosition(makeNewPosition(containerRef.current.clientWidth, containerRef.current.clientHeight));
      }
      animate();
    } else {
  ...
  return (
    <div
      ref={spriteRef}
      className={`fixed w-[75px] h-[75px] cursor-pointer transition-all duration-1000 ${className}`}
      style={{
        top: `${position[0]}px`,
        left: `${position[1]}px`,
